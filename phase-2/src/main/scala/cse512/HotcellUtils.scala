...
object HotcellUtils {
  ...
  def isCellWithinTheCube(x: Double, y: Double, z: Int, minX: Double, maxX: Double, minY: Double, maxY: Double, minZ: Int, maxZ: Int): Boolean = {
  ...
  def checkBoundaryOfPoint(point: Int, minVal: Int, maxVal: Int): Int = {
  ...
  def getCountOfNeighbour(minX: Int, minY: Int, minZ: Int, maxX: Int, maxY: Int, maxZ: Int, Xin: Int, Yin: Int, Zin: Int): Int = {
    ...
    initialState += checkBoundaryOfPoint(Xin, minX, maxX)
    initialState += checkBoundaryOfPoint(Yin, minY, maxY)
    initialState += checkBoundaryOfPoint(Zin, minZ, maxZ)

    var location = pointLocationInCube.get(initialState).get.toString()
